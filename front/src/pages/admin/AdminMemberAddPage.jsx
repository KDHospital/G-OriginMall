...
const AdminMemberAddPage = () => {
  ...
  const [form, setForm] = useState({ loginId: '', mname: '', mpwd: '', mpwdConfirm: '', tel: '', email: '', gender: 0 });
  ...
  const errCls = (f) => hasErr(f) ? 'border-red-300 focus:border-red-300 focus:ring-red-100' : 'border-gray-200 focus:border-blue-300 focus:ring-blue-100';
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await adminCreateMember({ loginId: form.loginId.trim(), mname: form.mname.trim(), mpwd: form.mpwd, tel: form.tel.trim(), email: form.email.trim(), gender: form.gender });
      alert("회원이 등록되었습니다."); navigate('/admin/members');
  ...
  return (
    <AdminLayout>
      <div className="flex-1 p-8 bg-[#f8f9fa] min-h-screen">
        <div className="mb-4 text-sm text-gray-400">회원 관리 &gt; 일반회원 &gt; <span className="text-gray-600 font-semibold">회원 등록</span></div>
        <div className="space-y-5">
          <div className="flex justify-between items-end">
            <div>
              <button onClick={() => navigate('/admin/members')} className="text-sm text-gray-400 hover:text-gray-600 transition-colors mb-1 flex items-center gap-1"><span>←</span> 목록으로</button>
              <h2 className="text-2xl font-bold text-gray-900">회원 등록</h2>
            </div>
            <div className="flex gap-2">
              <button onClick={() => navigate('/admin/members')} className="px-5 py-2.5 text-sm font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">취소</button>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div><Label text="이름" required /><input name="mname" value={form.mname} onChange={handleChange} type="text" placeholder="홍길동" className={cls('mname')} />{errMsg('mname') && <p className="text-xs text-red-500 mt-1.5">{errMsg('mname')}</p>}</div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div><Label text="성별" /><select name="gender" value={form.gender} onChange={handleChange} className={`${ic} bg-white border-gray-200 focus:border-blue-300 focus:ring-blue-100`}><option value={0}>미지정</option><option value={1}>남성</option><option value={2}>여성</option></select></div>
              </div>
            </div>
          </div>
        </div>
