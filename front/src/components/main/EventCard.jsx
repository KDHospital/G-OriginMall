        ...
        <section class="py-16 max-w-7xl mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {eventCardItem.map((item)=>(
                    <Link key={item.id} to={item.path} class="group relative h-80 rounded-2xl overflow-hidden shadow-lg cursor-pointer">
                        ...
                        <div class="absolute inset-0 p-8 flex flex-col justify-end items-end">
                            <Link class="bg-white text-primary px-6 py-2 rounded-lg font-bold w-fit hover:bg-secondary hover:text-accent transition-colors">페이지 바로가기</Link>
                        </div>
                    </Link> 
                ))}
            </div>
