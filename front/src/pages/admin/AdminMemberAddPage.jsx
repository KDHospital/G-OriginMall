...
const AdminMemberAddPage = () => {
  ...
  const validatePwd = () => {
    const newErrors = {};
    if (form.mpwd && !pwdRegex.test(form.mpwd)) {
      newErrors.mpwd = '8~20자, 영문과 숫자 또는 특수문자(!@#$%^&*)를 포함해야 합니다.';
    }
    if (form.mpwdConfirm && form.mpwd !== form.mpwdConfirm) {
      newErrors.mpwdConfirm = '비밀번호가 일치하지 않습니다.';
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!pwdRegex.test(form.mpwd)) return alert("비밀번호는 8~20자이며 영문과 숫자 또는 특수문자를 포함해야 합니다.");
  ...
  return (
    <AdminLayout>
      <div className="flex-1 p-8 bg-[#f8f9fa] min-h-screen">
        <div className="mb-4 text-sm text-gray-400">회원 관리 &gt; <span className="text-gray-600 font-semibold">회원 등록</span></div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-6 space-y-5">
              <div>
                ...
                <input name="loginId" value={form.loginId} onChange={handleChange} type="text" placeholder="example@email.com"
                  className="w-full max-w-md px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all" />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">이름</label>
                  <input name="mname" value={form.mname} onChange={handleChange} type="text" placeholder="홍길동"
                ...
                <div>
                  ...
                  <input name="email" value={form.email} onChange={handleChange} type="email" placeholder="example@email.com"
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input name="tel" value={form.tel} onChange={handleChange} type="text" placeholder="010-0000-0000" maxLength={13}
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">성별</label>
                  <select name="gender" value={form.gender} onChange={handleChange}
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none bg-white focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all">
